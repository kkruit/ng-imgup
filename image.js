...
angular.module('kkruit.directives').
  directive('kkImgUp', ['$parse', function ($parse) {
    return {
      restrict: 'E',
      require: 'ngModel',
      replace: true,
      template:   '<div class="clearfix">\n' +
              ...
              '<img kk-src="dispImg" class="img-rounded" />\n' +
  ...
  .directive('kkSrc', ['$parse', function ($parse) {
    // Runs during compile
    return {
      restrict: 'A', // E = Element, A = Attribute, C = Class, M = Comment
      link: function ($scope, iElm, iAttrs, controller) {
        var updateSrc = function () {
          var src = $parse(iAttrs.kkSrc)($scope);
          if (!src) {
        ...
        $scope.$watch(iAttrs.kkSrc, updateSrc);
        updateSrc();
      }
    };
  }]);
